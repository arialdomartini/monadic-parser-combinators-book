...
So defined, the details about passing `rest` and handling errors are not
directly visible from the outside. Good for information hiding. A
drawback of having the function wrapped inside `Parser`, though, is that
...
function that #emph[opens] it and reveals its content. This is a common
theme when working with monads: even if monads are not boxes --- nor
burritos --- sometimes the idea of #emph[opening a monad];, operating
with its content, and then #emph[wrapping] the result back in another
Monad comes in handy. \
Fine. The `Parser` type and the `run` function are an excellent starting
point.

== A Grammar For A Parsing Language
<a-grammar-for-a-parsing-language>
Let's give a look to the ending point, too. Over the course of the next
chapters, we will develop several functions and operators. They will
form the syntactic elements of a grammar with which you'll be able to
build whatever parser.
...
With a bit of fantasy, you can imagine that this is an actual operator:
...
operator name. But wait a moment! F\# #emph[does provide] an actual
...
But this is a complete nonsense! Think about it: an `int Parser` is not
an `int` value; it's #emph[a promise] of an `int`. Put your ear up to it
and you'll hear it whispering:

#quote(block: true)[
True, I am not an actual value myself. But I promise I can provide you
with one. \
Just feed me with an input string: I will do my best to parse it and, if
I don't fail, I will eventually give you back that much-desired `int`
value.
]
...
#quote(block: true)[
I see what you want to do! You want to multiply some integer value by
`2`. \
You know that we don't have that integer value just yet; you also know
we will eventually obtain one as soon as you feed with an input string.

Let's make a deal: pass me the calculation you want to run. I will parse
that input string for you, then I will apply that function. I promise.

I mean: you gave me a Parser, I pay you back with another Parser. It's
only fair.
]
...
will not compile. We cannot compare the returned `'b Parser` with `84`.
...
- But look! You do have a `string` input, because you live inside a
  lambda!
- So, just #emph[run] `aP` with `input` and you will get an
  `a   ParseResult`. If it is successful, you will find the `'a` value
...
Not bad! And not too difficult, after all. This function is interesting
because it captures the #emph[effectful logic];:

- It passes the unconsumed input forward.
- It deals with failures.

It does not contain any logic other than this. It really keeps concerns
separate:
...
next function. As we'll see shortly, however, when it comes to `<<|`, we
will prefer a different interpretation, one that involves mapping a
...
later. First, let's see a couple of examples how you can use these brand
new operators.

== What An Epic Time To Live!
<what-an-epic-time-to-live>
Let's cover this use case: you want to parse an input in a specific way,
but you can't get to the desired result all at once. Instead, first, you
parse the input into a value that is not exactly what you want. Then, in
each of the next steps, you apply a function to transform that value to
something else, possibly of a different type, until you get the final
type you had in mind. Of course, since we are talking about parser
combinators, you'd like to combine all these steps into a single,
composed parser.
...
measuring time as the number of second since where #emph[you] were born?
...
more convenient and intuitive representation than
`01 January 2025 18:11:12`. Only, now we need an `EpicTime Parser`.
...
<mapping-strings-to-types>
Let's see an other simple example based on the very same idea: applying
a function to the value eventually parsed by a parser or, anticipating
the lingo you will encounter, #emph[mapping] functions to parsers. \
...
trivial parser would return the specific string, if it was found in the
input; otherwise, it would fail. Later, you can refine it to return
specific custom types, by the means of `|>>`.
...
Keep `str` in mind: we are going to use it over and over. You can use it
as a building block to parse more refined parsers. For example, here's
...
Generally: `<<|` lets you apply a function to "the content" of a box.
This box is called Functor and `<<|` is also called `map`. You will
often hear expressions such as "A Functor is something you can map
over".

The second --- more fascinating and powerful --- interpretation arises
...
`map` is that combinator that given a function `f : 'a -> 'b` operating
on ordinary values #emph[lifts] it to work on parsers, as an
`'a Parser -> 'b Parser` function. It maps things from the lower world
to the elevated, Parser-powered universe.
...
- It is transformed to
  `toEpicTimeP: Parser DateTime -> Parser   EpicTime`.
...
Wow… That was a mouthful, wasn't it? Not only are Functors incredibly
powerful, but they are also pervasive in Functional Programming, forming
a fundamental building block. During your journey, you will surely
encounter them in many other contexts.

We've covered many details about transforming a single parser and
